...
## Corpus Check
- 88 files · ~44,932 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1096 nodes · 1672 edges · 72 communities (59 shown, 13 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 215 edges (avg confidence: 0.55)
...
Cohesion: 0.05
Nodes (59): ADR-006 Breadth Engine.  Computes market-wide breadth indicators from normalized, FeatureHandler, FeatureStore, Protocol, FeatureStore — authoritative state owner per ADR-004.  Ingest → route → handler, Deprecated: use get_symbol_state(). Accepts str exchange for backward compat., Authoritative state owner. Ingest, route, compute, store, notify., CandleHandler (+51 more)
...
Cohesion: 0.16
Nodes (15): Enum, EventType, Exchange, Event schemas — Phase 1 contract (frozen).  Hierarchy:   Event (base, all events, Registry lifecycle event — symbol added or removed., SymbolEvent, _infer_sector(), Any (+7 more)
...
Cohesion: 0.08
Nodes (9): Handler, EventBus, PerSymbolOrderedBus, Wraps EventBus with per-symbol ordering guarantee., Async priority-based pub/sub bus.      - High-priority events (trade, liquidatio, Register a handler. events/symbols set = filter; None = all., Start the delivery loop., MockExchangeConnection (+1 more)
...
Cohesion: 0.08
Nodes (25): BreadthEngine, Return fresh RawFeature or None if stale / missing., Snapshot of store health., MarketBreadth, NormalizedFeature, NormalizedFeatureUpdateEvent, RankedSymbol, One normalised feature value for one symbol. (+17 more)
...
### Community 76 - "Event"
Cohesion: 0.31
Nodes (5): PrioritizedEvent, Enqueue a normalised event for delivery., Event, Base event — every system event carries these., TestPrioritizedEvent
...
- **Why does `EventBus` connect `EventBus` to `MarketEvent`, `ObservabilityStore`, `TestSequenceValidator`, `Exchange`, `Event`, `ConnectionStatus`, `__init__.py`?**
  _High betweenness centrality (0.030) - this node is a cross-community bridge._
- **Why does `MarketEvent` connect `MarketEvent` to `binance.py`, `TestSequenceValidator`, `Exchange`, `Event`, `ConnectionStatus`, `EventBus`, `Event`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
- **Why does `Exchange` connect `Exchange` to `MarketEvent`, `binance.py`, `TestSequenceValidator`, `api.py`, `Event`, `ConnectionStatus`, `EventBus`, `Event`, `__init__.py`?**
...
- **Are the 17 inferred relationships involving `Exchange` (e.g. with `FeatureHandler` and `FeatureStore`) actually correct?**
  _`Exchange` has 17 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 21 inferred relationships involving `FeatureId` (e.g. with `BreadthEngine` and `FeatureHandler`) actually correct?**
  _`FeatureId` has 21 INFERRED edges - model-reasoned connections that need verification._